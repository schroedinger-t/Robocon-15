...
volatile int straycount2=0, temp=0;
int state;
int brake = -1;
int Lx_new;
int Ly_new;
...
volatile byte d[21]; //array of previous values
volatile int k;
...
void loop() {
  ...
  if(shaftcount!=0||shaftcount2!=0)
  {
    Serial.print("Junk ");
    Serial.print(shaftcount);
    Serial.print(" times ");
    Serial.print("Old values used ");
    Serial.print(shaftcount2);
    Serial.print(" times ");
  }
  BTSerial.write('s');
  attachInterrupt(contInt, receiveArray, CHANGE);
...
//    Serial.println();
  ...
  {
    Serial.println("Flag High, press R1");
  }
...
void receiveArray()
{
  ...
  if((a[12]==255)&&(a[13]==255)&&(a[14]==255)&&(a[15]==255)&&(a[16]==255))
    straycount++;
//  if(a[12]!=0 && a[12]!=1)
//  {
//    temp=1;
